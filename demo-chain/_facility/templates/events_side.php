...
if (isset($app)):
	...
	<div class="con_side">
		<div class="box_side box_cat">
			...
			<ul>
				<li><a href="<?php echo LOCATION_FACILITY . 'events/'; ?>"<?php echo (empty($currentCategory) )? ' class="active"' : "" ?>><?php echo $view['translator']->trans('common.all_posts', [], 'events'); ?>（<?php echo $cms__categories['total']; ?>）</a></li>
				<?php foreach ($cms__categories['categories'] as $category): ?>
					<li>
						<a href="<?php echo LOCATION_FACILITY . 'events/?' . http_build_query(['category' => $category['entity']->getId()], null, '&amp;'); ?>"<?php echo (!empty($currentCategory) && $currentCategory == $category['entity']->getId())? ' class="active"' : "" ?>>
						...
						<li><span class="accordion<?php echo ( !empty($currentYear) && $currentYear == $year )? ' active' : "" ?>"><?php echo $year; ?>（<?php echo $archive['total']; ?>）</span>
							<ul<?php echo ( !empty($currentYear) && $currentYear == $year )? ' style="display:block"' : "" ?>>
								...
								foreach ($archive['month'] as $month => $cnt):
									...
									?>
									<li><a href="<?php echo LOCATION_FACILITY . 'events/' . '?' . http_build_query(['year' => $year, 'month' => $month], null, '&amp;'); ?>"<?php echo ( (!empty($currentYear) && !empty($currentMonth)) && ($currentYear == $year && $currentMonth == $month) )? ' class="active"' : "" ?>><?php echo $year; ?>/<?php echo $month; ?>（<?php echo $cnt['count']; ?>）</a></li>
								<?php endforeach; ?>
							</ul>
						</li>
					<?php endforeach; ?>
