...
document.addEventListener("DOMContentLoaded", function (e) {
	...
	if (!$('#js-sliderBanner').is('.slick-slider') && abi.sp) {
		$('#js-sliderBanner').slick({
			...
			pauseOnFocus: false,
			pauseOnHover: false,
			appendArrows: $('#js-banArrow'),
			appendDots: $('#js-banArrow'),
	...
	$w.on({
		...
		resizeAfter: function () {
			...
			//con_offer
			$match_offer = $('.con_offers .box_offers .list_offers li a').find('.tit_plan');
			$match_offer2 = $('.con_offers .box_offers .list_offers li a').find('.txt_price');
			matchHeight($match_offer,(abi.pc) ? 3 : (abi.tab) ?3 : 1);
			matchHeight($match_offer2,(abi.pc) ? 3 : (abi.tab) ?3 : 1);
		}
	}).firstLoad({
		//firstLoad
		pc_tab: function () {
